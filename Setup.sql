...
-- レポート分析専用データベースの作成
CREATE OR REPLACE DATABASE COPORATE_REPORT_ANALYZE;
USE DATABASE CORPORATE_REPORT_ANALYZE;

-- レポート検索用スキーマの作成
CREATE SCHEMA REPORT_SERCH_SCHEMA;
USE SCHEMA REPORT_SERCH_SCHEMA;
...
CREATE OR REPLACE GIT REPOSITORY COPORATE_REPORT_ANALYZE.REPORT_SERCH_SCHEMA.WORKSHOP_AI_USECASE_REPO
  API_INTEGRATION = GIT_API_INTEGRATION
  ORIGIN = 'https://github.com/snow-jp-handson-org/Workshop_AI_UseCase_Discovery.git';

-- リポジトリの最新コードを取得
ALTER GIT REPOSITORY COPORATE_REPORT_ANALYZE.REPORT_SERCH_SCHEMA.WORKSHOP_AI_USECASE_REPO FETCH;

-- =========================
-- 2. Git Repo から内部ステージへファイルをコピー
-- =========================
COPY FILES
  INTO @COPORATE_REPORT_ANALYZE.REPORT_SERCH_SCHEMA.FILES
  FROM @COPORATE_REPORT_ANALYZE.REPORT_SERCH_SCHEMA.WORKSHOP_AI_USECASE_REPO/branches/main/Reports/Constract_Report/
  PATTERN = '.*\.pdf';
...
-- レポートメタデータ管理テーブル (レポート名・タイトル・HTML本文を保持)
CREATE TABLE IF NOT EXISTS COPORATE_REPORT_ANALYZE.REPORT_SERCH_SCHEMA.HTML_REPORT_METADATA (
    REPORT_NAME VARCHAR NOT NULL,   -- ファイル識別子 (英数字+アンダースコアのみ)
    TITLE VARCHAR,                  -- 表示用タイトル (日本語可)
    HTML_CONTENT VARCHAR,           -- HTML本文 (最大16MB)
    CREATED_AT TIMESTAMP_NTZ DEFAULT CURRENT_TIMESTAMP()  -- 作成/更新日時
);

-- Agentが呼び出すカスタムツール本体
-- 機能: HTMLコンテンツをステージにアップロードし、メタデータテーブルにUPSERT
CREATE OR REPLACE PROCEDURE COPORATE_REPORT_ANALYZE.REPORT_SERCH_SCHEMA.DEPLOY_HTML_REPORT(
...
def main(session, html_content: str, report_name: str, title: str) -> str:
    # バリデーション: ファイル名は英数字とアンダースコアのみ許可
    if not re.match(r''^[a-zA-Z0-9_]+$'', report_name):
        return "Error: report_name must contain only alphanumeric characters and underscores."

    # バリデーション: HTML本文が空または極端に短い場合はエラー
    ...
    # ステージへのアップロード (失敗してもテーブル保存で継続する設計)
    try:
        input_stream = io.BytesIO(html_content.encode(''utf-8''))
        session.file.put_stream(
            input_stream,
            "@^^w.ANALYZE.HTML_REPORTS/" + file_name,
            auto_compress=False,   -- 圧縮なし (HTMLをそのまま保存)
            overwrite=True         -- 同名ファイルは上書き
        )
    except Exception as e:
        pass  -- ステージ書き込み失敗時もテーブル保存を優先

    # SQLインジェクション対策: シングルクォートのエスケープ
    safe_rn = report_name.replace("''", "''''")
    safe_t = title.replace("''", "''''")
    safe_html = html_content.replace("''", "''''")

    # メタデータテーブルへのUPSERT (MERGE文で既存レポートは上書き、新規は挿入)
    merge_sql = (
        "MERGE INTO COPORATE_REPORT_ANALYZE.REPORT_SERCH_SCHEMA.HTML_REPORT_METADATA AS target "
        "USING (SELECT ''" + safe_rn + "'' AS REPORT_NAME, ''" + safe_t + "'' AS TITLE, ''" + safe_html + "'' AS HTML_CONTENT) AS source "
        "ON target.REPORT_NAME = source.REPORT_NAME "
        "WHEN MATCHED THEN UPDATE SET TITLE = source.TITLE, HTML_CONTENT = source.HTML_CONTENT, CREATED_AT = CURRENT_TIMESTAMP() "
        "WHEN NOT MATCHED THEN INSERT (REPORT_NAME, TITLE, HTML_CONTENT) VALUES (source.REPORT_NAME, source.TITLE, source.HTML_CONTENT)"
    )
...
-- CREATE OR REPLACE AGENT COPORATE_REPORT_ANALYZE.REPORT_SEARCH_SCHEMA.REPORT_ANALYSIS_AGENT
...
--
-- ツール定義:
--   cortex_search : Cortex Search Serviceへの全文検索クエリ実行
--   web_search    : インターネット検索 (最新情報取得用)
--   generic       : カスタムストアドプロシージャの呼び出し
--
-- tools:
--   - tool_spec:
--       type: "cortex_search"
...
--     max_results: 1000
--     search_service: "COPORATE_REPORT_ANALYZE.REPORT_SERCH_SCHEMA.REPORT_SEARCH_SERVICE"
...
--   deploy_html_report:
--     identifier: "COPORATE_REPORT_ANALYZE.REPORT_SERCH_SCHEMA.DEPLOY_HTML_REPORT"
--     type: "function"
...
-- スキーマ切り替え
USE DATABASE COPORATE_REPORT_ANALYZE;
USE SCHEMA ANALYZE;
...
CREATE OR REPLACE STREAMLIT COPORATE_REPORT_ANALYZE.REPORT_SERCH_SCHEMA.HTML_VIEWER
  ROOT_LOCATION = '@COPORATE_REPORT_ANALYZE.REPORT_SERCH_SCHEMA.WORKSHOP_AI_USECASE_REPO/branches/main/html_viewer/'  -- Gitリポジトリ内のアプリディレクトリ
...
-- - Git FETCHはStreamlitコード更新時に再実行が必要
